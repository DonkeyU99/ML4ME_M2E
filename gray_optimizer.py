...
from local_prior_2d import smooth_region
import cv2
...
class Optimizer:
    def __init__(self, image, kernel_size, sigma=None, max_iterations=15):
        self.I_origin = image
        self.I = cv2.cvtColor(image, cv2.COLOR_RGB2GRAY)
        self.I_flat = self.I.flatten().T
        self.I_grad_x = compute_gradient(self.I, "x")
        self.I_grad_y = compute_gradient(self.I, "y")
        self.I_grad_x_flat = self.I_grad_x.flatten().T
        self.I_grad_y_flat = self.I_grad_y.flatten().T
        self.I_grad_xx_flat = compute_gradient(self.I, "xx").flatten().T
        self.I_grad_xy_flat = compute_gradient(self.I, "xy").flatten().T
    ...
    def gradient_filter(self, type):
        ...
        if type == 'x':
            filter = np.array([[1, 0, -1], [2, 0, -2], [1, 0, -1]])/4
                        
        if type == 'y':
            filter = np.array([[1, 2, 1], [0, 0, 0], [-1, -2, -1]])/4

        if type == 'xy':
            filter = np.array([[1, 0, -1], [0, 0, 0], [-1, 0, 1]])/2

        if type == 'xx':
            filter = np.array([[1, 2, 1], [-2, -4, -2], [1, 2, 1]])/4

        if type == 'yy':
            filter = np.array([[1, -2, 1], [2, -4, 2], [1, -2, 1]])/2
        return filter
    ...
    def get_argmin_Psi(self, mask_3d):
        ...
        solution_x = np.choose(
            np.argmax([positive_values, negative_values, quadratic_values], axis=0),
    ...
    def update_L(self):
        ...
        new_L = np.zeros((self.height, self.width))

        numer = np.conjugate(F_f) * self.F_I * self.delta + self.gamma * (
            np.conjugate(self.sigma_star[1]) * F_psi_x
...
#
    ...
    def optimize(self):
        ...
        plt.imshow(cv2.cvtColor(self.L,cv2.COLOR_GRAY2RGB))
